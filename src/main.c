...
usage(FILE *stream)
{
	fprintf(stream, "Usage: ./miniexcel <input.csv>\n");
}

char *
slurp_file(const char *file_path, size_t *size)
{
	char *buffer = NULL;
	...
	assert(n == (size_t) m);	// amount of stuff we read is amount of stuff excepted
